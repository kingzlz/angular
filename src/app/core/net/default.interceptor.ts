...
export class DefaultInterceptor implements HttpInterceptor {
  ...
  private toLogin(error?: string): void {
    this.notification.error(error || `未登录或登录已过期，请重新登录。`, ``);
    ...
    switch (ev.status) {
      ...
      default:
        if (ev instanceof HttpErrorResponse) {
          ...
          //   event,
          // );
          const msg: string = (ev.error && ev.error.message) || ev.statusText || '';
          if (msg) {
            this.msg.error(msg);
          }
        }
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    newReq = this.reAttachToken(newReq);
